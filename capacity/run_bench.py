...
def main(argv):
    ...
    if "rhhi-perf1" in hostname: 
        os.system("ssh root@rhhi-cap1.lab.eng.blr.redhat.com echo 3 > /proc/sys/vm/drop_caches")
        os.system("ssh root@rhhi-cap2.lab.eng.blr.redhat.com echo 3 > /proc/sys/vm/drop_caches")
        os.system("ssh root@rhhi-cap3.lab.eng.blr.redhat.com echo 3 > /proc/sys/vm/drop_caches")

    # Set up the DB connection
    try:
        conn = MySQLdb.connect(host= "10.70.53.10", user="root", db="benchmarking_capacity")
	...
	if populate == 1:
	   uid = "db_population" 
        ...
        #cmd = 'curl -X POST http://admin:redhat@34.233.15.254:3000/api/annotations -H "Content-Type: application/json" -d \'{"dashboardId":' + "6" + ',"time":' + str(starttime) + ',"isRegion":true,"timeEnd":' + str(endtime) + ',"tags":["' + tags + '"],"text":"' + idstring + '"}\''
        cmd = 'curl -X POST http://admin:redhat@34.233.15.254:3000/api/annotations -H "Content-Type: application/json" -d \'{"dashboardId":' + "6" + ',"time":' + str(starttime) + ',"isRegion":true,"timeEnd":' + str(endtime) + ',"tags":["' + uid + '", "capacity"],"text":"' + idstring + '"}\''
